...
def test_between(lower_limit, upper_limit, item):
    assert lower_limit <= item <= upper_limit, "{2} is not between {0} and {1}" .format(lower_limit, upper_limit, item)
...
def test_exception_was_raised(func, args, message):
    ...
    try:
        ...
        assert e.args[0] == message, "The message that was provided did not match the message thrown. '{0}' does not match '{1}'".format(e.args[0], message)
